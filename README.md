...
系统支持学生在一封邮件中提交多个不同作业：

- **自动识别**：从邮件主题、附件文件名、邮件正文中识别多作业提交
- **智能分类**：自动将附件归属到对应的作业
- **综合确认**：发送包含所有作业的确认邮件
- **严格模式**：确保所有附件都有明确归属，否则标记为人工审核

配置选项：
- `ENABLE_MULTI_ASSIGNMENT`: 启用/禁用多作业功能
- `MIN_CONFIDENCE_THRESHOLD`: AI识别置信度阈值
- `STRICT_MODE`: 严格模式开关

使用方式：
- 方式1：在主题中说明 "2021001张三-作业1、作业2提交"
- 方式2：使用清晰的文件名 "作业1报告.pdf"、"作业2代码.zip"
- 方式3：在正文中说明各附件归属

详细文档：[多作业功能说明](docs/MULTI_ASSIGNMENT_FEATURE.md)

### 数据恢复与修正

- **智能重试**: 一键重试当前页面的所有失败条目
- **批量AI重析**: 使用最新邮件内容重新提取选中条目的信息
