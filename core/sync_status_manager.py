...
class SyncStatusManager(BaseStatusManager):
    """同步状态管理器"""
    ...
    def transition(
        ...
        try:
            # 获取当前状态
            current_status = self.get_status(submission_id, status_type)
            ...
            # 更新兼容的旧status字段
            self._update_legacy_status(submission_id)
    ...
    def transition_batch(
        ...
        for submission_id in submission_ids:
            if self.transition(submission_id, status_type, new_status, reason):
                results['success'] += 1
            else:
                results['failed'] += 1
        ...
        try:
            ...
            if result:
                return {
                    ...
                    'ai_status': result[1],
                    'download_status': result[2],
                    'reply_status': result[3]
                }
            return {}
    ...
    def reset_to_retry(
        ...
        """重置为可重试状态"""
        try:
            statuses = self.get_all_statuses(submission_id)
            if not statuses:
                return False
    ...
    def _update_legacy_status(self, submission_id: int):
        """更新兼容的旧status字段"""
        try:
            statuses = self.get_all_statuses(submission_id)
...
def get_sync_status_manager(db_session):
    """获取同步状态管理器实例"""
    global _sync_status_manager
    if _sync_status_manager is None:
        _sync_status_manager = SyncStatusManager(db_session)
